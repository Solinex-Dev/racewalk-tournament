...
export function HeadJudgeJoinForm({ eventId, event }: HeadJudgeJoinFormProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    // TODO: ภายหลังเชื่อมต่อ API ตรวจสอบ head_judge_code ที่ถูกต้อง
    setSubmitting(true);
    setTimeout(() => {
      setSubmitting(false);
      if (!eventId) return;
      // mock: ถ้ากรอกอะไรมาก็ให้ไปหน้า head-judge
      router.push(`/head-judge/events/${eventId}`);
    }, 400);
  };
        ...
        <CardContent className="space-y-6">
          <div className="space-y-2 text-center">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-400">
              การแข่งขันเดินทน – หัวหน้ากรรมการ
            </p>
            <h1 className="text-2xl font-semibold text-slate-100">
              เข้าร่วมเป็นหัวหน้ากรรมการใน Event
            </h1>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-3">
              <label className="block text-sm font-medium text-slate-200">
                รหัสลับของหัวหน้ากรรมการใน Event นี้
              </label>
            ...
            >
              {submitting ? "กำลังตรวจสอบรหัส..." : "เข้าร่วม Event ในฐานะหัวหน้ากรรมการ"}
            </Button>
          </form>
