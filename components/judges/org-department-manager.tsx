...
export function OrgDepartmentManager({ organizations }: { organizations: OrgNode[] }) {
  ...
  const renameOrg = (org: OrgNode) => {
    const name = window.prompt("เปลี่ยนชื่อองค์กร", org.name)?.trim();
    if (!name || name === org.name) return;
    run(() => updateOrganization(org.id, { name }), "เปลี่ยนชื่อองค์กรเรียบร้อย");
  };

  const renameDept = (orgId: string, dept: { id: string; name: string }) => {
    const name = window.prompt("เปลี่ยนชื่อแผนก", dept.name)?.trim();
    if (!name || name === dept.name) return;
    run(() => updateDepartment(dept.id, { name }), "เปลี่ยนชื่อแผนกเรียบร้อย");
  };

  const removeOrg = (org: OrgNode) => {
    if (!window.confirm(`ลบองค์กร "${org.name}" และแผนกทั้งหมดภายใต้องค์กรนี้?`)) return;
    run(() => deleteOrganization(org.id), "ลบองค์กรเรียบร้อย");
  };

  const removeDept = (dept: { id: string; name: string }) => {
    if (!window.confirm(`ลบแผนก "${dept.name}"?`)) return;
    run(() => deleteDepartment(dept.id), "ลบแผนกเรียบร้อย");
  };
