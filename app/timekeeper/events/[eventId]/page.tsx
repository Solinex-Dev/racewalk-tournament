...
export const metadata: Metadata = {
  title: "หน้าคนนับรอบ – การแข่งขันเดินทน",
};
...
export default async function TimekeeperPage(props: Props) {
  const { eventId } = await props.params;
  const session = await getOfficialSession();

  if (!session || session.eventId !== eventId || session.position !== "TIMEKEEPER") {
    redirect(`/timekeeper/events/${eventId}/join`);
  }
  ...
  if (!round) redirect(`/timekeeper/events/${eventId}/join`);
  ...
  return (
    <>
      ...
      <LapRecorder
        ...
        roleLabel="คนนับรอบ"
        joinPath={`/timekeeper/events/${eventId}/join`}
